...
const SearchPage = () => {
  ...
  const handleSearch = () => {
    ...
    setResults(mockResults.filter(result => result.toLowerCase().includes(query.toLowerCase())));
  };
    ...
    <div className="min-h-screen flex flex-col items-center p-6 font-sans">
      ...
      <ul className="list-none p-0 w-full max-w-md mt-6 bg-white rounded-lg shadow-md">
        {results.map((result, index) => (
          <li key={index} className="p-3 border-b border-gray-200 last:border-none">{result}</li>
        ))}
      </ul>
    </div>
  );
